...
##Initial setting
local_path <- "/Desktop/youho/cg/" #this refers to where the textural data files are on your local disk
...
tRawAllToken_CG.melt<-melt(tRawAllToken_CG.df, id="year", value.name="tokenCG",na.rm=TRUE)
tRawAllToken_CG.melt<-tRawAllToken_CG.melt[,c(2,1,3)]
colnames(tRawAllToken_CG.melt)<-c("code", "year","tokenCG")
...
tRawTTRNun_CG.melt<-melt(tRawTTRNun_CG.df, id="year", value.name="ttrCG",na.rm=TRUE)
tRawTTRNun_CG.melt<-tRawTTRNun_CG.melt[,c(2,1,3)]
colnames(tRawTTRNun_CG.melt)<-c("code","year","ttrCG")
...
dbGetQuery(con, "drop table if exists cgTTR")
rs=dbWriteTable(con,"cgTTR",as.data.frame(rawCG_TokenTTR.melt),row.names=T)
dbGetQuery(con,"SELECT * FROM cgTTR limit 5;")
dbListTables(con)
```
...
colnames(tRawTfidif_CG.melt)<-c("code", "year", "tfidfCG")
```
...
driver=dbDriver("SQLite")
con=dbConnect(driver,dbname)
dbGetQuery(con, "drop table if exists cgTfidf")
rs=dbWriteTable(con,"cgTfidf",as.data.frame(tRawTfidif_CG.melt),row.names=T)
#dbGetQuery(con,"SELECT * FROM cgTfidf limit 5;")
#dbListTables(con)
```